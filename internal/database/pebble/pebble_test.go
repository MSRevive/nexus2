...
func seedUser(t *testing.T, db *pebbleDB, steamid string) {
	t.Helper()
...
func seedCharacter(t *testing.T, db *pebbleDB, steamid string, slot, size int, data string) uuid.UUID {
	t.Helper()
...
func flush(t *testing.T, db *pebbleDB) {
	t.Helper()
...
func TestGetUser_LoadsDeletedCharacters(t *testing.T) {
	db := newTestDB(t)
	charID := seedCharacter(t, db, "steam1", 0, 100, "data")
	//_, errrrrrrrrr := db.GetCharacter(charID)
	//require.NoError(t, errrrrrrrrr)
...
func TestGetUser_CharacterMapsAreInitialized(t *testing.T) {
	...
	assert.Nil(t, u.DeletedCharacters)
}
